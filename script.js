...
function printItems (items, wrapper, page, limit) {
    wrapper.innerHTML = '';
    page--;

    let start = page * limit;
    let end = start + limitCount;
    let paginationArray = items.slice(start, end);
    
    paginationArray.forEach((elem, index) => {
        main.innerHTML += `
            <div class="card card${index}">
                ...
                <button onclick="addPopUp(${index})">BUY</button>
            </div>
        `
    })

    paginatonButtons(arr, paginationBtn);
}

function paginatonButtons (items, wrapper) {
    wrapper.innerHTML = '';

    let buttonsCount = Math.ceil(items.length / limitCount);
...
function changePage (page) {
    pageCount = page;
    printItems(arr, main, pageCount, limitCount);
    window.scroll({
        top: 100,
        left: 100,
        behavior: "smooth",
...
form.addEventListener('submit', (e) => {
    ...
    let filtredArray = arr.filter((elem) => elem?.brand?.includes(input.value.trim()) || elem?.title?.includes(input.value.trim()));
    printItems(filtredArray, main, pageCount, limitCount);
})

function addPopUp (index) {
    if (!popupArr.includes(arr[index])) {
        popupArr.push(arr[index]);
    }

    printPopUpArr(popupArr, popUpItems);
}

function printPopUpArr (popupData, wrapper) {
    wrapper.innerHTML = '';
    popupData.forEach((elem, index) => {
        wrapper.innerHTML += `
            <div class="item">
                <button>-</button>
                <img src="${elem.thumbnail}">
